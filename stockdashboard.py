...
st.set_page_config(page_title="통합 그룹핑 투자 대시보드", layout="wide")
...
st.title("📊 초정밀 그룹핑 투자 대시보드")
st.markdown("거시 경제 흐름부터 첨단 밸류체인(전력망, 소부장, 스마트팩토리)까지 시장 자금의 이동을 추적합니다.")

period_option = st.sidebar.radio("조회 기간 선택", ["1개월", "3개월", "6개월", "1년", "3년"], index=1)
...
if not df.empty:
    ...
    # [수정됨] 에러 방지용 안전장치 (데이터 수집 성공한 항목만 기본값으로 설정)
